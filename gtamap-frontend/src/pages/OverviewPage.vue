<template>
  <div class="tw:flex tw:flex-col">
    ...
    <div class="tw:flex tw:flex-row tw:gap-6 tw:justify-center tw:p-6">
      <div class="tw:w-fit my-card tw:mx-6 tw:my-6">
        <h3 class="tw:text-lg tw:font-bold tw:mb-4 tw:text-center">
          Mapper Agreement on Mapping Locations
        </h3>
        <div class="tw:flex tw:justify-center">
          <UpsetPlotMapperAgreement title="" url="/api/upsetData" @onSetClick="handleUpsetPlotSetClick"
            @onSetHover="handleUpsetPlotSetHover"></UpsetPlotMapperAgreement>
        </div>
      </div>
    </div>
    ...
    <div class="my-card tw:mx-6 tw:my-6">
      ...
      <ReadSummaryTable ref="readSummaryTableRef" class="tw:flex-1" @open-read-details="openReadDetailsPage"
        @accepted-table-update="tableUpdated" @summary-table-update="summaryTableUpdate">
      </ReadSummaryTable>
      <Igv ref="igvSummary" url="/api/summary/igvConfig" :callback="setIgvBrowserSummary" @read-click="handleReadClick">
      </Igv>
    </div>

    <div class="my-card tw:mx-6 tw:my-6">
      <h3 class="tw:text-lg tw:font-bold tw:mb-4 tw:text-center">Accepted Reads</h3>
      <Igv ref="igvAccepted" url="/api/accepted/igvConfig" :callback="setIgvBrowserAccepted"
        @read-click="handleReadClickAccepted"></Igv>
    </div>
...
export default {
  ...
  setup() {
    ...
    const handleUpsetPlotSetClick = function (set) {

      // const qnames = set.elems.map(e => {
      //   return e.name.split("::")[1]
      // })

      // DataService.filterForSpecific(set.name, qnames)
    }
    const handleUpsetPlotSetHover = function (set) {
      // console.log('Hovered set:', set)
    }
    ...
    return {
      ...
      handleUpsetPlotSetClick,
      handleUpsetPlotSetHover,
    }
  },
  mounted() { },
}
