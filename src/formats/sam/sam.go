...
type Entry struct {
	...
	TranscriptId int    // id of the transcript that this read was aligned to
}

func (entry *Entry) String() string {
	alignmentLineString := fmt.Sprintf("%s\t%d\t%s\t%d\t%d\t%s\t%s\t%d\t%d\t%s\t%s", entry.Qname, entry.Flag, entry.Rname, entry.Pos, entry.Mapq, entry.Cigar, entry.Rnext, entry.Pnext, entry.Tlen, entry.Seq, entry.Qual)
