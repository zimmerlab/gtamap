...
func readPairResultToSamString(
	...
	// TAGS
	tagsFw := ""
	if resFw != nil && resFw.OccurrenceWeightHarmonic > 0 && resFw.OccurrenceWeightGeometric > 0 {
		tagsFw = fmt.Sprintf(
			"XH:i:%d\tXG:i:%d",
	...
	tagsRv := ""
	if resRv != nil && resRv.OccurrenceWeightHarmonic > 0 && resRv.OccurrenceWeightGeometric > 0 {
		tagsRv = fmt.Sprintf(
			"XH:i:%d\tXG:i:%d",
