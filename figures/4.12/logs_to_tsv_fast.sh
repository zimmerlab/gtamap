...
parse_file() {
    ...
    awk -v file="$base" -v tool="$tool" '
        ...
        /Command being timed:/ {
        # extract gene starting with ENGS followed by digits
            match($0, /ENSG[0-9]+/, m)
            gene = m[0]
        }
...
if command -v parallel >/dev/null 2>&1; then
    find "$logdir" -type f -name "*.e*" | parallel -j$(nproc) parse_file {}
else
